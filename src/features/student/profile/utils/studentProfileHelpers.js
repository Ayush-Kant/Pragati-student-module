...
export const calculateProfileCompletion = (profile) => {
  ...
  const checkDocuments = () => {
    let docScore = 0;
    if (profile.resume && profile.resume.trim() !== '') docScore += PROFILE_COMPLETION_WEIGHTS.DOCUMENTS * 0.5;
...
export const getCompletionStepStatus = (stepId, profile) => {
  switch (stepId) {
    ...
    case PROFILE_SECTIONS.DOCUMENTS: {
      return (profile.resume && profile.resume.trim() !== '') || (profile.documents && profile.documents.length > 0);
    }
...
export const getMissingFields = (profile) => {
  ...
  if (!profile.resume || profile.resume.trim() === '') missing.push({ section: PROFILE_SECTIONS.DOCUMENTS, field: 'resume', label: 'Resume' });
