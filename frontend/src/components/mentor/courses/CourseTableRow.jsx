...
export default function CourseTableRow({ course, onArchive, onView, onEdit }) {
  const categoryClass = CATEGORY_STYLES[course.category] || "bg-gray-100 text-gray-700";

  return (
    <tr className="rounded-2xl border border-gray-100 bg-white shadow-sm">
      <td className="rounded-l-2xl bg-white p-4">
        <div className="flex items-center gap-4">
          <img src={course.image} alt={course.title} className="h-16 w-16 rounded-2xl object-cover" />
          <div>
            <h3 className="font-semibold text-gray-900">{course.title}</h3>
            <p className="mt-1 max-w-xs text-sm text-gray-500 line-clamp-2">{course.description}</p>
          </div>
        </div>
      </td>

      <td className="bg-white p-4">
        <span className={`rounded-full px-4 py-2 text-nowrap text-sm font-medium ${categoryClass}`}>
          {course.category}
        </span>
      </td>
      ...
      <td className="bg-white p-4">
        <div className="flex items-center gap-3">
          <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(course.mentor)}`} alt={course.mentor} className="h-11 w-11 rounded-full" />
          <div>
      ...
      <td className="bg-white p-4 font-medium text-gray-700">{course.students}</td>
