...
import { CalendarClock, CheckCircle2, ExternalLink, Video, Eye, RefreshCw } from 'lucide-react';
...
export default function InterviewsPage() {
  ...
  const confirm = async (id) => {
    ...
    setError('');
    try {
      await api.post(`/student/interviews/${id}/confirm`);
      await load();
    } catch (err) {
  ...
  const loadOutcome = async (id) => {
    ...
    setError('');
    try {
      const response = await api.get(`/student/interviews/${id}/outcome`);
      setOutcomeById((current) => ({ ...current, [id]: response.data?.outcome || response.data?.data || null }));
    } catch (err) {
      const message = err?.response?.data?.message || err?.message || 'Unable to load interview outcome.';
      setError(message);
    } finally {
      setBusyId(null);
    }
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {interviews.map((interview) => {
            const outcome = outcomeById[interview.id];
            return (
              <article key={interview.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                  ...
                  {interview.result && String(interview.result).toUpperCase() !== 'PENDING' && (
                    <button
                      type="button"
                      onClick={() => loadOutcome(interview.id)}
                      disabled={busyId === interview.id}
                    ...
                    >
                      {busyId === interview.id ? <RefreshCw className="animate-spin" size={16} /> : <Eye size={16} />}
                      View outcome
                    </button>
                  )}
                </div>
                ...
                {outcome && (
                  <div className="mt-5 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm text-slate-700">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-semibold">Outcome:</span>
                      <span className="capitalize">{String(outcome.status || '').toLowerCase().replaceAll('_', ' ')}</span>
                    </div>
                    {outcome.feedback && <p className="mt-1 text-slate-600">{outcome.feedback}</p>}
                    {outcome.offerLetterUrl && (
                      <a
                        href={outcome.offerLetterUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="mt-3 inline-flex items-center gap-2 font-semibold text-blue-700 hover:text-blue-800"
                      >
                        <ExternalLink size={15} /> Download offer letter
                      </a>
                    )}
                    {outcome.publishedAt && (
                      <p className="mt-2 text-xs text-slate-500">Published {formatDate(outcome.publishedAt)}</p>
                    )}
                  </div>
                )}
