...
export default function SM02ProfileWorkspace() {
  ...
  return <div className="min-h-full bg-slate-50 px-3 py-5 sm:px-5 lg:px-8"><div className="mx-auto max-w-6xl space-y-5">
    <header className="overflow-hidden rounded-3xl bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-700 p-5 text-white shadow-lg sm:p-7"><div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-4"><div className="shrink-0">{personal.profileImage || personal.avatarUrl ? <img src={personal.profileImage || personal.avatarUrl} alt={personal.name || "Student"} className="h-20 w-20 rounded-2xl object-cover ring-4 ring-white/20" /> : <div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white/15 text-2xl font-black ring-4 ring-white/10">{initials(personal.name)}</div>}</div><div><div className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-100">Student profile • SM-02</div><h1 className="mt-1 text-2xl font-black text-white sm:text-3xl">{personal.name || "Complete your profile"}</h1><p className="mt-1 text-sm text-blue-100">{personal.email || "Add your account details"}</p><div className="mt-3 flex flex-wrap gap-2 text-xs">{academic.course ? <span className="rounded-full bg-white/10 px-3 py-1">{academic.course}</span> : null}{academic.department ? <span className="rounded-full bg-white/10 px-3 py-1">{academic.department}</span> : null}{academic.batch ? <span className="rounded-full bg-white/10 px-3 py-1">Batch {academic.batch}</span> : null}</div></div></div><button type="button" onClick={() => { setError(""); setSuccess(""); setEditing(true); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-blue-700 shadow-sm hover:bg-blue-50"><Pencil className="h-4 w-4" />Edit profile</button></div></header>
