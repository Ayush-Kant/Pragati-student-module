...
const statusStyles = {
  Submitted: 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-300 dark:border-emerald-900',
  Pending: 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-300 dark:border-blue-900',
  Overdue: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/30 dark:text-red-300 dark:border-red-900',
      ...
      <StudentPageShell>
        <div className="animate-pulse space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {Array.from({ length: 4 }, (_, index) => <div key={index} className="h-40 rounded-2xl bg-slate-200 dark:bg-slate-800" />)}
          </div>
        </div>
      </StudentPageShell>
    );
    ...
    <StudentPageShell>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <div key={label} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
            <Icon className="mb-2 text-blue-600 dark:text-blue-400" size={18} />
      ...
      <div className="mb-6 space-y-3">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm text-slate-900 outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:focus:ring-blue-950"
          />
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {filtered.map((assignment) => (
            <article key={assignment.id} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-800">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-blue-600 dark:text-blue-400">{assignment.subject}</p>
                  <h2 className="truncate text-lg font-semibold text-slate-900 dark:text-slate-100">{assignment.title}</h2>
                </div>
              ...
              <p className="mb-5 line-clamp-2 text-sm leading-6 text-slate-600 dark:text-slate-400">
              ...
              <button
                ...
                className="mt-4 w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-slate-900"
              >
                View assignment
              </button>
            </article>
