...
import { getAssignmentById, getAssignmentFeedback, getAssignmentSubmission, submitAssignment } from '../../services/assignment.service';
  ...
  const load = async () => {
    ...
    try {
      const [a, s, f] = await Promise.all([
        ...
        getAssignmentFeedback(id).catch(() => null),
      ]);
      setAssignment(a);
      setSubmission(s);
      setFeedback(f);
      setContent(s?.content || '');
      setExternalLink(s?.fileName ? '' : s?.fileUrl || '');
    } catch(err) {
      setError(err?.response?.data?.message || err?.message || 'Unable to load assignment.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return <StudentPageShell>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
      <section className="space-y-6">
        ...
        {(assignment.grade || assignment.feedback || feedback || submission) && <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-base font-bold text-slate-900">Submission result</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-slate-50 p-4"><div className="text-xs text-slate-500">Status</div><div className="mt-1 font-bold text-slate-900">{submission?.status || 'Not submitted'}</div></div><div className="rounded-xl bg-slate-50 p-4"><div className="text-xs text-slate-500">Attempt</div><div className="mt-1 font-bold text-slate-900">{attemptsUsed}{assignment.maxResubmissions ? ` / ${Number(assignment.maxResubmissions)+1}` : ''}</div></div></div>{submission?.lateDays > 0 && <div className="mt-3 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">Late by {submission.lateDays} day(s). Penalty applied: {submission.latePenalty}%.</div>}{assignment.grade && <div className="mt-3 rounded-xl bg-emerald-50 p-4 text-sm text-emerald-800"><strong>Score: {assignment.grade.score}/{assignment.totalMarks}</strong>{assignment.grade.remarks ? ` • ${assignment.grade.remarks}` : ''}</div>}{feedback && <div className="mt-3 rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-indigo-900"><div className="font-bold">Mentor feedback</div>{feedback.generalComment && <p className="mt-1 whitespace-pre-wrap">{feedback.generalComment}</p>}{feedback.totalScore != null && <p className="mt-2 text-sm font-semibold">Total score: {feedback.totalScore}/{assignment.totalMarks}</p>}{feedback.gradedAt && <p className="mt-1 text-xs text-indigo-700">Graded {formatDate(feedback.gradedAt)}</p>}{Array.isArray(feedback.rubric) && feedback.rubric.length > 0 && <div className="mt-3 space-y-2">{feedback.rubric.map((criterion, index) => <div key={`${criterion.criterion || 'criterion'}-${index}`} className="rounded-lg bg-white/80 p-3 text-xs"><div className="font-semibold">{criterion.criterion || `Criterion ${index + 1}`}{criterion.score != null && criterion.maxScore != null ? ` — ${criterion.score}/${criterion.maxScore}` : ''}</div>{criterion.comment && <p className="mt-1 text-indigo-800">{criterion.comment}</p>}</div>)}</div>}{feedback.resubmissionAllowed && <p className="mt-3 text-xs font-semibold text-emerald-700">A resubmission is available.</p>}</div>}{assignment.feedback && !feedback && <div className="mt-3 rounded-xl bg-indigo-50 p-4 text-sm text-indigo-900"><div className="font-bold">Mentor feedback</div><p className="mt-1 whitespace-pre-wrap">{assignment.feedback.remarks}</p>{assignment.feedback.grade && <p className="mt-2 text-xs font-semibold">Rubric grade: {assignment.feedback.grade}</p>}{Array.isArray(assignment.feedback.inlineComments) && assignment.feedback.inlineComments.length > 0 && <div className="mt-3 space-y-2">{assignment.feedback.inlineComments.map((comment, index) => <div key={index} className="rounded-lg bg-white/70 p-2.5 text-xs text-indigo-900">{typeof comment === 'string' ? comment : comment.comment || comment.text || JSON.stringify(comment)}</div>)}</div>}</div>}{submission?.fileUrl && <a href={submission.fileUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-blue-600"><ExternalLink size={14}/> Open submitted work{submission.fileName ? ` (${submission.fileName})` : ''}</a>}</div>}
