...
const EligibilityCriteria = ({ eligibility = null, className = "" }) => {
  ...
  const scoreProgress =
    scoreRequired > 0 ? clamp((scoreValue / scoreRequired) * 100) : 100;
  ...
  const submissionsProgress =
    requiredCount > 0
      ? clamp((submittedCount / requiredCount) * 100)
      : submissionsComplete
        ? 100
        : 0;

  const attendanceValue = Number(
    eligibility.attendance?.attendancePercent ?? 0,
  );
  const attendanceRequired = Number(
    eligibility.attendance?.requiredAttendance ?? 60,
  );
  const attendanceProgress =
    attendanceRequired > 0
      ? clamp((attendanceValue / attendanceRequired) * 100)
      : 100;
  ...
  const criteria = [
    ...
    },
    {
      ...
      detail:
        requiredCount > 0
          ...
          : "No mandatory activities configured",
    ...
    },
    {
      ...
      detail: eligibility.drive?.name
        ? `${eligibility.drive.name}: ${eligibility.drive.status || "not completed"}`
        : "Recruitment drive must be completed",
      ...
      {criteria.map(({ key, label, detail, value, completed, icon: Icon }) => (
        ...
        >
          ...
          <div className="mt-4">
            ...
            <p className="mt-2 min-h-10 text-[11px] leading-5 text-slate-500 dark:text-slate-400 sm:text-xs">
              {detail}
            </p>
