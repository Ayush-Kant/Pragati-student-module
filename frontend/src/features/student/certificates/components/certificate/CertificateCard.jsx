...
const CertificateCard = ({
  ...
  const issued = isCertificateIssued(certificate);
  const verified = isCertificateVerified(certificate);
  const canShare = Boolean(certificate.verificationUrl || certificate.verifyUrl);

  const handleView = () => {
    if (typeof onView === "function") onView(certificate);
  };

  const handleDownload = () => {
    if (typeof onDownload === "function" && issued && !downloadLoading) {
          ...
          <div className="relative">
            <div className="flex items-start justify-between gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white text-slate-700 shadow-sm ring-1 ring-slate-200 sm:h-12 sm:w-12">
                <Award className="h-5 w-5 sm:h-6 sm:w-6" strokeWidth={1.8} aria-hidden="true" />
              </div>

              <CertificateStatusBadge status={status} size="sm" />
            </div>

            <div className="mt-5">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400">Certificate</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Issued on</p>
                <p className="mt-0.5 text-sm font-semibold text-slate-700">{formatCertificateDate(issueDate)}</p>
              </div>
            </div>

            <div className="rounded-xl border border-slate-100 bg-slate-50 px-3.5 py-3">
              <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Certificate ID</p>
            ...
            {certificate.verificationCode ? (
              <div className="rounded-xl border border-slate-100 bg-white px-3.5 py-3">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Verification code</p>
                <p className="mt-1 break-all font-mono text-xs font-semibold tracking-wide text-slate-700 sm:text-sm">
                  {certificate.verificationCode}
                </p>
              </div>
            ...
            <div className="flex items-center justify-between gap-3 rounded-xl border border-slate-100 px-3.5 py-3">
              <div className="flex min-w-0 items-center gap-2.5">
                <ShieldCheck className={`h-4 w-4 shrink-0 ${verified ? "text-emerald-600" : "text-slate-400"}`} aria-hidden="true" />
                <span className="truncate text-xs font-medium text-slate-600 sm:text-sm">Verification</span>
              </div>
              <CertificateStatusBadge status={verificationStatus} type="verification" />
            </div>
          </div>
          ...
          <div className="mt-auto pt-5">
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
              ...
              <button
                ...
                disabled={!canShare}
                className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-xs font-semibold text-slate-700 transition-colors duration-200 hover:bg-slate-50 hover:text-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-200 focus:ring-offset-2 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400 sm:text-sm"
