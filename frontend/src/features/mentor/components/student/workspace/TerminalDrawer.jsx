      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-[#FFFFFF]">
        {isLoading ? (
          ...
          <div className="max-w-4xl space-y-4 md:space-y-6">
            ...
            {/* Verdict Header (flex-wrap ensures it doesn't break on narrow devices) */}
            <div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm font-mono">
              ...
              <span className="text-[#6B7280]">Runtime: <strong className="text-[#111827]">{result.executionTimeMs} ms</strong></span>
              <span className="text-[#6B7280]">Memory: <strong className="text-[#111827]">{result.memoryMB} MB</strong></span>
            </div>

            {/* Score & Cases Metric Boxes */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <div className="bg-[#F8FAFC] border border-[#E5E7EB] p-4 md:p-5 rounded-md">
                <p className="text-[10px] md:text-xs text-[#6B7280] font-semibold mb-1 md:mb-2 uppercase tracking-wider">Score</p>
                <div className="text-xl md:text-2xl font-bold text-[#111827]">
                  {result.totalScore} <span className="text-sm md:text-base text-[#9CA3AF] font-medium">/ {result.maxScore}</span>
                </div>
              </div>
              <div className="bg-[#EFF6FF] border border-[#BFDBFE] p-4 md:p-5 rounded-md">
                <p className="text-[10px] md:text-xs text-[#6B7280] font-semibold mb-1 md:mb-2 uppercase tracking-wider">Passed Cases</p>
                <div className="text-xl md:text-2xl font-bold text-[#111827]">
                  {result.passedTestCases} <span className="text-sm md:text-base text-[#9CA3AF] font-medium">/ {result.totalTestCases}</span>
                </div>
              </div>
            </div>
