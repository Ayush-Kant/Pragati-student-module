...
export const CompanyAnalyticsChart = ({ data = [], darkMode = false }) => {
  ...
  return (
    <div className={`p-4 rounded-xl border h-64 ${darkMode ? "bg-[#2D2D2D] border-[#3D3D3D]" : "bg-white border-gray-100"}`}>
      <h4 className={`text-xs font-bold uppercase tracking-wider mb-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Company Placements</h4>
      <ResponsiveContainer width="100%" height="90%">
        <BarChart data={data} layout="vertical" margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? "#3D3D3D" : "#F3F4F6"} horizontal={false} />
          <XAxis type="number" fontSize={10} tickLine={false} axisLine={false} stroke={darkMode ? "#6B7280" : "#9CA3AF"} />
          <YAxis dataKey="company" type="category" fontSize={10} tickLine={false} axisLine={false} stroke={darkMode ? "#6B7280" : "#9CA3AF"} width={70} />
