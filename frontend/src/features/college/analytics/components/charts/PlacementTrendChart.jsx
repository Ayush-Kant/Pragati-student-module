...
export const PlacementTrendChart = ({ data = [], darkMode = false }) => {
  ...
  return (
    <div className={`p-4 rounded-xl border h-64 ${darkMode ? "bg-[#2D2D2D] border-[#3D3D3D]" : "bg-white border-gray-100"}`}>
      <h4 className={`text-xs font-bold uppercase tracking-wider mb-2 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>Placement Trends</h4>
      <ResponsiveContainer width="100%" height="90%">
        <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="placed" stroke="#2563eb" fill={darkMode ? "rgba(37,99,235,0.15)" : "#EEF2FF"} strokeWidth={2} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  );
