...
import { Eye, UserPlus, CircleUserRound, CheckSquare } from "lucide-react";
...
const NominationTable = ({
  ...
  const fullGrid = `grid grid-cols-[${checkboxCol}2.4fr_1.3fr_1.7fr_0.7fr_1fr_1.3fr_0.9fr]`;
  const compactGrid = `grid grid-cols-[${checkboxCol}3fr_1.5fr_1fr]`;
      ...
      <div className="h-142 overflow-y-auto">
        {!compactView ? (
          <div className={`${fullGrid} sticky top-0 z-10`}>
            {onToggleSelect && <div className={`${headerClass} justify-center`}><CheckSquare size={14} /></div>}
                ...
                <div className={`${rowClass} justify-center`}>
                  <div className="flex items-center justify-center gap-2">
                    <button
                      ...
                      title="View Details"
                      className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-200 ${selectedStudent?.id === student.id && isDetailOpen
