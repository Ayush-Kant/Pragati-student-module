...
export const reorderLessonsInModule = async ({ mentorId, moduleId, lessonOrder }) =>
  withClient(async (client) => {
    ...
    try {
      ...
      if (inputLessonIds.length !== moduleLessonIds.length) {
        const err = new Error('All lessons in the module must be included in lessonOrder');
        err.statusCode = 400;
        throw err;
      }
...
export const addLessonToModule = async ({
  ...
  withClient(async (client) => {
    ...
    try {
      ...
      const prerequisiteIds = Array.isArray(prerequisites) ? [...new Set(prerequisites.map(Number))] : [];
      ...
      if (prerequisiteIds.length > 0) {
        ...
        if (validPrereqs.length !== prerequisiteIds.length) {
          const err = new Error('prerequisites must be valid lesson IDs within the same course');
          err.statusCode = 400;
          throw err;
        }
      }
...
export const updateLessonById = async ({ mentorId, lessonId, payload }) =>
  withClient(async (client) => {
    ...
    try {
      ...
      if (payload.videoUrl !== undefined && payload.videoUrl !== lesson.video_url) {
        if (lesson.video_url) {
      ...
      const updatedTitle = payload.title !== undefined ? payload.title : lesson.title;
      const updatedHlsUrl = payload.hlsUrl !== undefined ? payload.hlsUrl : lesson.hls_url;
      const updatedThumbnailUrl = payload.thumbnailUrl !== undefined ? payload.thumbnailUrl : lesson.thumbnail_url;
      const updatedDuration = payload.duration !== undefined ? payload.duration : lesson.duration_sec;
      const updatedStatus = payload.status !== undefined ? payload.status : lesson.status;
...
export const checkLessonAccessByUser = async ({ userId, lessonId }) =>
  withClient(async (client) => {
    ...
    const requiredLessons = Array.isArray(lessonRows[0].prerequisites)
      ? lessonRows[0].prerequisites.map(Number).filter((n) => Number.isInteger(n))
      : [];
    ...
    /*
      Adjust this query/table name if your project uses a different progress table.
      This version assumes a lesson_progress table:
      - student_id
      - lesson_id
      - completed_at
    */
    ...
    const completedSet = new Set(completedRows.map((row) => Number(row.lesson_id)));
    const hasAccess = requiredLessons.every((lessonIdValue) => completedSet.has(lessonIdValue));
