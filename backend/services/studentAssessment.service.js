...
class StudentAssessmentService {
  ...
  async startAssessment(studentId, assessmentIdValue) {
    ...
    try {
      ...
      const attemptResult = await client.query(`INSERT INTO student_assessment_attempts (assessment_id,student_id,attempt_number,started_at,expires_at,total_marks) VALUES ($1,$2,$3,NOW(),NOW()+($4*INTERVAL '1 minute'),$5) RETURNING *`, [assessmentId,studentId,nextAttempt,timeLimit,totalMarks]);
      ...
      for(let index=0; index<questions.length; index+=1){
        ...
        const optionOrder=(normalizeQuestionType(question.type)==="MCQ"||normalizeQuestionType(question.type)==="TRUE_FALSE")?(assessment.shuffle_options!==false?[...options.keys()].sort(()=>Math.random()-0.5):options.map((_option,optionIndex)=>optionIndex)):[];
  ...
  async saveAnswer(studentId, attemptIdValue, questionIdValue, answer){
    const attemptId=parseAttemptId(attemptIdValue);const questionId=Number(questionIdValue);
    ...
    try{
      ...
      const question=questionResult.rows[0];const type=normalizeQuestionType(question.type);let normalized=answer;
      if(type==="MCQ"){const optionOrder=Array.isArray(question.option_order)?question.option_order.map(Number):[];const options=normalizeOptions(question.options);const index=Number(answer?.optionIndex);if(!Number.isInteger(index)||index<0||index>=optionOrder.length)throw Object.assign(new Error("Selected option is invalid for this attempt"),{statusCode:422});normalized={optionIndex:index,optionId:options[optionOrder[index]]?.id??`option_${optionOrder[index]}`};}
      else if(type==="TRUE_FALSE"){const raw=answer?.value??answer?.answer??answer;if(typeof raw!=="boolean"&&!['true','false'].includes(String(raw).trim().toLowerCase()))throw Object.assign(new Error("True/False answer is invalid"),{statusCode:422});normalized={value:typeof raw==='boolean'?raw:String(raw).trim().toLowerCase()==='true'};}
      else if(type==="FILL_BLANK"){const text=String(answer?.text??answer?.value??"").trim();if(!text||text.length>500)throw Object.assign(new Error("Fill-in-the-blank answer must be 1-500 characters"),{statusCode:422});normalized={text};}
      else if(type==="MATCH"){const matches=answer?.matches&&typeof answer.matches==="object"?answer.matches:{};if(!Object.keys(matches).length)throw Object.assign(new Error("Match-the-following answer is required"),{statusCode:422});normalized={matches};}
      else throw Object.assign(new Error(`Unsupported assessment question type: ${question.type}`),{statusCode:422});
      await persistAnswer(client,attemptId,questionId,normalized);await client.query("COMMIT");return {saved:true,attemptId:`attempt_${attemptId}`,questionId};
    }catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}
  }

  async recordTabSwitch(studentId,attemptIdValue){const attemptId=parseAttemptId(attemptIdValue);const result=await pool.query(`UPDATE student_assessment_attempts SET tab_switch_count=tab_switch_count+1,updated_at=NOW() WHERE id=$1 AND student_id=$2 AND status='in_progress' AND expires_at>NOW() RETURNING tab_switch_count`,[attemptId,studentId]);if(!result.rows[0])throw Object.assign(new Error("Attempt not found or no longer active"),{statusCode:409});return {attemptId:`attempt_${attemptId}`,tabSwitchCount:Number(result.rows[0].tab_switch_count)};}

  async submitAssessment(studentId,attemptIdValue,reason="submitted",answersPayload=null){
    const attemptId=parseAttemptId(attemptIdValue);const client=await pool.connect();
    try{
      await client.query("BEGIN");
      ...
      if(Array.isArray(answersPayload)){
        for(const entry of answersPayload){if(!entry)continue;const questionId=Number(entry.questionId??entry.question_id);if(!Number.isInteger(questionId)||questionId<=0)continue;const questionResult=await client.query(`SELECT aq.*,saq.option_order FROM student_assessment_attempt_questions saq JOIN assessment_questions aq ON aq.id=saq.question_id WHERE saq.attempt_id=$1 AND aq.id=$2`,[attemptId,questionId]);if(!questionResult.rows[0])continue;const q=questionResult.rows[0];const type=normalizeQuestionType(q.type);let normalized=entry.answer;if(type==="MCQ"){const optionOrder=Array.isArray(q.option_order)?q.option_order.map(Number):[];const options=normalizeOptions(q.options);const index=Number(entry.answer?.optionIndex);if(!Number.isInteger(index)||index<0||index>=optionOrder.length)throw Object.assign(new Error("Selected option is invalid for this attempt"),{statusCode:422});normalized={optionIndex:index,optionId:options[optionOrder[index]]?.id??`option_${optionOrder[index]}`};}else if(type==="TRUE_FALSE"){const raw=entry.answer?.value??entry.answer?.answer??entry.answer;if(typeof raw!=="boolean"&&!['true','false'].includes(String(raw).trim().toLowerCase()))throw Object.assign(new Error("True/False answer is invalid"),{statusCode:422});normalized={value:typeof raw==='boolean'?raw:String(raw).trim().toLowerCase()==='true'};}else if(type==="FILL_BLANK"){const text=String(entry.answer?.text??entry.answer?.value??"").trim();if(!text||text.length>500)throw Object.assign(new Error("Fill-in-the-blank answer must be 1-500 characters"),{statusCode:422});normalized={text};}else if(type==="MATCH"){const matches=entry.answer?.matches??{};if(!matches||typeof matches!=="object"||!Object.keys(matches).length)throw Object.assign(new Error("Match-the-following answer is required"),{statusCode:422});normalized={matches};}else throw Object.assign(new Error(`Unsupported assessment question type: ${q.type}`),{statusCode:422});await persistAnswer(client,attemptId,questionId,normalized);}}
      const questions=await getAttemptQuestions(client,attemptId);const requiredIds=questions.filter((question)=>question.is_required!==false).map((question)=>Number(question.id));const storedAnswers=await getStoredAnswers(client,attemptId);const missing=requiredIds.filter((id)=>!storedAnswers.has(id));const expired=attempt.status==='expired'||new Date(attempt.expires_at)<=new Date();const auto=expired||String(reason).toLowerCase()==='timeout';if(missing.length&&!auto)throw Object.assign(new Error(`Answer all required questions before submitting (${missing.length} remaining)`),{statusCode:400});
      const grading=gradeAssessment(questions,storedAnswers);const percentage=grading.totalMarks>0?Number(((grading.score/grading.totalMarks)*100).toFixed(2)):0;const passed=percentage>=Number(attempt.passing_percentage||PASSING_DEFAULT);const status=auto?'auto_submitted':'submitted';const updated=await client.query(`UPDATE student_assessment_attempts SET status=$1,submitted_at=NOW(),score=$2,total_marks=$3,percentage=$4,passed=$5,updated_at=NOW() WHERE id=$6 RETURNING *`,[status,grading.score,grading.totalMarks,percentage,passed,attemptId]);const updatedAttempt=updated.rows[0];
      ...
      await client.query("COMMIT");return this.getResult(studentId,updatedAttempt.id);
    }catch(error){await client.query("ROLLBACK");throw error;}finally{client.release();}
  }
  ...
  async getResult(studentId,attemptIdValue){
    ...
    const counts=await pool.query(`SELECT COUNT(saq.question_id)::INTEGER AS total_questions, COUNT(*) FILTER (WHERE sa.is_correct)::INTEGER AS correct_answers FROM student_assessment_attempt_questions saq LEFT JOIN student_assessment_answers sa ON sa.attempt_id=saq.attempt_id AND sa.question_id=saq.question_id WHERE saq.attempt_id=$1`,[attemptId]);
    const reviewGate=row.review_available_at||row.due_at;const reviewAvailable=Boolean(row.review_enabled)&&(!reviewGate||new Date(reviewGate)<=new Date());const started=new Date(row.started_at).getTime();const submitted=row.submitted_at?new Date(row.submitted_at).getTime():Date.now();
  ...
  async getReview(studentId,assessmentIdValue){
    const assessmentId=parseAssessmentId(assessmentIdValue);const attemptResult=await pool.query(`SELECT saa.id,saa.attempt_number,a.title,a.review_enabled,a.review_available_at,a.due_at FROM student_assessment_attempts saa JOIN assessments a ON a.id=saa.assessment_id WHERE saa.assessment_id=$1 AND saa.student_id=$2 AND saa.status IN ('submitted','auto_submitted') ORDER BY saa.attempt_number DESC LIMIT 1`,[assessmentId,studentId]);
    if(!attemptResult.rows[0])throw Object.assign(new Error("No completed assessment attempt found"),{statusCode:404});const attempt=attemptResult.rows[0];if(!attempt.review_enabled)throw Object.assign(new Error("Answer review is not enabled for this assessment"),{statusCode:403});const gate=attempt.review_available_at||attempt.due_at;if(gate&&new Date(gate)>new Date())throw Object.assign(new Error(`Answer review becomes available at ${new Date(gate).toISOString()}`),{statusCode:403});
  ...
  async getHistory(studentId){const result=await pool.query(`SELECT saa.id,saa.assessment_id,saa.attempt_number,saa.status,saa.started_at,saa.submitted_at,saa.score,saa.total_marks,saa.percentage,saa.passed,a.title FROM student_assessment_attempts saa JOIN assessments a ON a.id=saa.assessment_id WHERE saa.student_id=$1 ORDER BY COALESCE(saa.submitted_at,saa.started_at) DESC`,[studentId]);return result.rows.map((row)=>({attemptId:`attempt_${row.id}`,assessmentId:`assess_${row.assessment_id}`,title:row.title,attemptNumber:Number(row.attempt_number),status:row.status,startedAt:row.started_at,submittedAt:row.submitted_at,score:Number(row.score||0),totalMarks:Number(row.total_marks||0),percentage:Number(row.percentage||0),passed:Boolean(row.passed)}));}
