...
const getStudentIdForUser = async (userId) => {
  const result = await pool.query(
    `SELECT id AS "studentId" FROM students WHERE user_id = $1 LIMIT 1`,
    [userId],
  );
  if (!result.rows[0]) {
...
export const getStudentPreferences = async (user) => {
  ...
  return Object.fromEntries(
    result.rows.map((row) => [
      ...
      { inApp: row.inApp, email: row.email, push: row.push },
    ]),
  );
};
...
export const getNotifications = async ({ userId, read = "all", page = 1, limit = 20 }) => {
  ...
  const readCondition = readFilter === 'all'
    ? ''
    : `AND n.is_read = ${readFilter === 'true' ? 'true' : 'false'}`;
...
export const sendNotification = async ({
  ...
  if (!targetUserIds.length) {
    return { success: true, message: "No users to notify", delivered: 0 };
  }
  ...
  const studentIdsByUser = new Map(studentRows.rows.map((row) => [Number(row.userId), Number(row.studentId)]));
  const preferenceRows = studentRows.rows.length
    ? await pool.query(
        `SELECT student_id, in_app, email, push
         FROM notification_preferences
         WHERE student_id = ANY($1::int[]) AND notification_type = $2`,
        [studentRows.rows.map((row) => Number(row.studentId)), type],
      )
    : { rows: [] };
  ...
  for (const targetUserId of targetUserIds) {
    const studentId = studentIdsByUser.get(targetUserId);
    const pref = preferenceByStudent.get(studentId) || DEFAULT_PREFERENCES[type];
    if (!pref.in_app && !(sendEmail && pref.email) && !(sendPush && pref.push)) continue;
    ...
    if (sendPush && pref.push) {
      pushUsers.push(targetUserId);
    }
  }
  ...
  if (pushUsers.length) {
    ...
    for (const userId of pushUsers) {
      await sendPushNotifications({
        subscriptions: grouped.get(userId) || [],
        payload: { title, body: message, type, linkUrl },
      });
    }
  }
...
export const sendDailyDigests = async () => {
  ...
  for (const row of candidates.rows) {
    const alreadySent = await pool.query(
      `SELECT 1 FROM notification_digest_log
       WHERE user_id = $1 AND digest_date = CURRENT_DATE
       LIMIT 1`,
      [row.userId],
    );
    ...
    await emailQueue.add({
      userId: row.userId,
      title: 'Your Pragati notification digest',
      message: `You have ${count} unread notification${count === 1 ? '' : 's'} waiting in Pragati.`,
      type: 'digest',
    });
    await pool.query(
      `INSERT INTO notification_digest_log (user_id, digest_date)
