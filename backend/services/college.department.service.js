...
export const getDepartments = async ({ page = 1, pageSize = 20, activeOnly = false } = {}) => {
  const limit = Math.min(Math.max(Number(pageSize) || 20, 1), 100);
  const offset = (Math.max(Number(page) || 1, 1) - 1) * limit;

  const [rows, total] = await Promise.all([
    departmentModel.getAllDepartments({ limit, offset, activeOnly }),
    departmentModel.countDepartments({ activeOnly }),
  ]);
...
export const addDepartment = async (payload) => {
  ...
  if (existingByCode) {
    const err = new Error(`Department code "${normalizedCode}" is already in use.`);
    err.statusCode = 409;
    throw err;
  }
  if (existingByName) {
    const err = new Error(`Department name "${name}" is already in use.`);
    err.statusCode = 409;
    throw err;
  }
...
export const editDepartment = async (id, payload) => {
  ...
  if (payload.code !== undefined) fields.code = payload.code.trim().toUpperCase();
  if (payload.hod !== undefined) fields.hod = payload.hod;
  ...
  if (fields.code && fields.code !== existing.code) {
    ...
    if (owner && owner.id !== Number(id)) {
      const err = new Error(`Department code "${fields.code}" is already in use.`);
      err.statusCode = 409;
      throw err;
    }
  }
  if (fields.name && fields.name !== existing.name) {
    ...
    if (owner && owner.id !== Number(id)) {
      const err = new Error(`Department name "${fields.name}" is already in use.`);
      err.statusCode = 409;
      throw err;
    }
  }

  const updated = await departmentModel.updateDepartment(id, fields);
...
export const searchDepartments = async (searchTerm, { page = 1, pageSize = 20 } = {}) => {
  const limit = Math.min(Math.max(Number(pageSize) || 20, 1), 100);
  const offset = (Math.max(Number(page) || 1, 1) - 1) * limit;
  const rows = await departmentModel.searchDepartments(searchTerm.trim(), { limit, offset });
