...
const getReports = async ({ limit = 20, offset = 0, type, status } = {}) => {
  const values = [];
  let query =
    'SELECT id, title, type, status, format, content, created_at AS "createdAt", updated_at AS "updatedAt" FROM generated_reports WHERE 1=1';
  if (type) {
    values.push(type);
...
const createReport = async (payload = {}) => {
  ...
  const result = await pool.query(
    `INSERT INTO generated_reports (title, type, status, format, content, created_by, created_at, updated_at) 
...
const getReportById = async (id) =>
  (
    await pool.query(
      'SELECT id, title, type, status, format, content, created_at AS "createdAt", updated_at AS "updatedAt" FROM generated_reports WHERE id = $1',
      [id],
    )
  ).rows[0] || null;
