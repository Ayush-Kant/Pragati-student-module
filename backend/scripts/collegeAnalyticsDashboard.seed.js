...
const seedAnalytics = async () => {
  ...
  try {
    ...
    console.log(
      "Clearing existing analytics tables for college ID:",
      collegeId,
    );
    await client.query(
    ...
    await client.query("DELETE FROM company_statistics WHERE college_id = $1", [
      collegeId,
    ]);
    await client.query("DELETE FROM monthly_reports WHERE college_id = $1", [
      collegeId,
    ]);
    await client.query("DELETE FROM yearly_reports WHERE college_id = $1", [
      collegeId,
    ]);

    // 3. Seed analytics_dashboard
    console.log("Inserting analytics_dashboard...");
    await client.query(
      ...
      [collegeId, 2450, 1280, 82.0, 12.0, "Google", 15, 45],
    );

    // 4. Seed placement_statistics (trends)
    console.log("Inserting placement_statistics...");
    ...
    console.log("Inserting company_statistics...");
    const companyStats = [
    ...
    for (const stat of companyStats) {
      ...
      await client.query(
        `INSERT INTO company_statistics (college_id, company_id, company_name, total_hired, average_package)
         VALUES ($1, $2, $3, $4, $5)`,
        [collegeId, companyId, stat.name, stat.hired, stat.avgPackage],
      );
    }

    // 6. Seed department_statistics
    console.log("Inserting department_statistics...");
    // Let's get department ids
    ...
    console.log("Inserting monthly_reports...");
    const months = [
      "2026-01",
      "2026-02",
      "2026-03",
    ...
    console.log("Inserting yearly_reports...");
    ...
    // 9. Update students table to link to our new collegeId and department names
    console.log("Updating students to link to collegeId...");
    await client.query(
      `UPDATE students 
       SET college_id = $1, college = $2
       WHERE id IN (1, 2, 3)`,
      [collegeId, "Pragati Institute of Technology"],
    );
