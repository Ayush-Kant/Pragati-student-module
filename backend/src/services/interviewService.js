...
export const createInterview = async (studentId, data) => {
  ...
  return await sequelize.transaction(async (transaction) => {
    ...
    for (const round of defaultRounds) {
      ...
      createdRounds.push(createdRound.toJSON ? createdRound.toJSON() : createdRound);
    }

    const result = interview.toJSON ? interview.toJSON() : interview;
    result.rounds = createdRounds;
    return result;
  });
};
