...
export const logActivity = async (projectId, userId, activityType, description, metadata = {}) => {
  try {
    ...
    // Activity log failure should not crash main operation
    return null;
  }
};
...
export const submitProject = async (projectId, submissionData, userId) => {
  ...
  // Calculate current submission version
  ...
  // Update repository URL on main project record
...
export const deleteProjectFile = async (projectId, fileId, userId) => {
  ...
  if (fs.existsSync(fileRecord.filePath)) {
    try {
      ...
      // Ignore disk delete error if file not present on filesystem
    }
  }
