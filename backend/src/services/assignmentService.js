...
class AssignmentService {
    static async createAssignment(input) {
        ...
        try {
            ...
            if (assignment.studentId) await notificationService.sendNotificationToStudents({ studentIds: [assignment.studentId], ...payload });
            else await notificationService.sendNotification({ role: 'student', ...payload });
        } catch (error) {
    ...
    static async listAssignments(filters = {}) { return assignmentModel.listAssignments(filters); }
    ...
    static async getSubmission(assignmentId, studentId) { return assignmentSubmissionModel.getSubmissionByAssignment(assignmentId, studentId); }
    static async listSubmissions(filters = {}) { return assignmentSubmissionModel.listAllSubmissions(filters); }
    static async getStatistics(filters = {}) { return assignmentModel.getAssignmentStatistics(filters); }
