...
export const getFeedback = async(req,res,next)=>{try{const studentId=await normalizeStudentId(req); const feedback=await AssignmentService.getFeedback(req.validatedParams.id,studentId); res.status(200).json({success:true,feedback,data:feedback});}catch(error){next(error);}};
...
export default {createAssignment,listAssignments,getAssignmentById,updateAssignment,deleteAssignment,submitAssignment,getSubmission,getFeedback,listSubmissions,getStatistics,addFeedback,addGrade};
