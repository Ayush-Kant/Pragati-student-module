...
const buildAssignment = (row) => buildAssignmentPayload({
  ...
  grade: row.grade_score !== null && row.grade_score !== undefined ? { score: Number(row.grade_score), remarks: row.grade_remarks, createdAt: row.grade_created_at } : null,
  feedback: row.feedback_remarks || row.feedback_grade || row.feedback_inline_comments ? { remarks: row.feedback_remarks, grade: row.feedback_grade, inlineComments: row.feedback_inline_comments, createdAt: row.feedback_created_at } : null,
});
...
export const listAssignments = async ({ studentId, status } = {}) => {
  ...
  if (requestedStatus === 'pending') conditions.push('s.id IS NULL AND a.status <> $2');
  else if (requestedStatus === 'submitted') conditions.push('s.id IS NOT NULL');
  else if (requestedStatus === 'graded') conditions.push('g.id IS NOT NULL');
  else if (requestedStatus === 'closed') { values.push(ASSIGNMENT_STATUS.CLOSED); conditions.push(`a.status = $${values.length}`); }
  else if (requestedStatus === 'open') { values.push(ASSIGNMENT_STATUS.OPEN); conditions.push(`a.status = $${values.length}`); }
...
export const getAssignmentStatistics = async ({ studentId } = {}) => {
  const result=await pool.query(`SELECT COUNT(*)::int AS total, COUNT(*) FILTER(WHERE a.status=$2)::int AS closed, COUNT(*) FILTER(WHERE a.status=$3)::int AS open, COUNT(*) FILTER(WHERE a.status=$4)::int AS pending, COUNT(DISTINCT s.assignment_id)::int AS submitted, COALESCE(AVG(g.score),0)::float AS "averageScore" FROM assignments a LEFT JOIN assignment_submissions s ON s.assignment_id=a.id AND s.student_id=$1 LEFT JOIN assignment_grades g ON g.assignment_id=a.id AND g.student_id=$1 WHERE a.student_id=$1 OR a.student_id IS NULL`,[studentId,ASSIGNMENT_STATUS.CLOSED,ASSIGNMENT_STATUS.OPEN,ASSIGNMENT_STATUS.PENDING]);
