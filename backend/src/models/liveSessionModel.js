...
const resolveDriveId = async (studentId) => {
  const result = await pool.query(
    `SELECT drive_id AS "driveId" FROM student_drive_progress WHERE student_id = $1 ORDER BY stage_updated_at DESC NULLS LAST, id DESC LIMIT 1`,
    [studentId],
  );
...
export const getAllSessions = async (studentId, filters = {}) => {
  ...
  const conditions = [driveId ? `(ls.drive_id = $2 OR ls.drive_id IS NULL)` : `ls.drive_id IS NULL OR ls.drive_id IS NOT NULL`];
  ...
  const result = await pool.query(
    `SELECT ls.id, ls.mentor_id AS "mentorId", ls.title, ls.session_type AS "sessionType",
            ...
            COALESCE(sa.attended, FALSE) AS attended, COALESCE(sa.status, 'Absent') AS "attendanceStatus",
            sa.attended_at AS "attendedAt", sr.recording_url AS "recordingUrl"
  ...
  return result.rows.map((row) => {
    ...
    const endsAt = start && mins > 0 ? new Date(start.getTime() + mins * 60 * 1000) : null;
    ...
    return {
      ...
      joinable: withinWindow && (row.status === 'Live' || (joinableAt && Date.now() >= joinableAt.getTime() && row.status !== 'Completed')),
...
export const getSessionById = async (id, studentId = null) => {
  ...
  const result = await pool.query(
    `SELECT ls.id, ls.mentor_id AS "mentorId", ls.title, ls.session_type AS "sessionType",
            ls.scheduled_at AS "scheduledAt", ls.trainer, ls.date, ls.time, ls.duration, ls.status,
            ls.drive_id AS "driveId", ls.meeting_url AS "meetingUrl", ls.room_name AS "roomName", ls.created_at AS "createdAt"
...
export const joinSession = async (sessionId, studentId, userName = 'Student', sessionDurationMinutes = 0) => {
  ...
  const result = await pool.query(
    `INSERT INTO session_participants (session_id,student_id,joined_at,left_at,join_token_issued_at)
     ...
     ON CONFLICT (session_id,student_id) DO UPDATE SET joined_at=NOW(),left_at=NULL,duration_seconds=NULL,
       join_token_issued_at=CASE WHEN $3 IS NOT NULL THEN NOW() ELSE session_participants.join_token_issued_at END
...
export const leaveSession = async (sessionId, studentId) => {
  const resolvedStudentId = await resolveStudentUserId(pool, studentId);
  if (!resolvedStudentId) throw new Error("Student not found");
  const existingAttendance = await pool.query('SELECT attended FROM session_attendance WHERE session_id=$1 AND student_id=$2', [sessionId, resolvedStudentId]);
  const previousAttended = Boolean(existingAttendance.rows[0]?.attended);
  const participant = await pool.query(
    `UPDATE session_participants
     SET left_at=COALESCE(left_at,NOW()), updated_at=NOW(), duration_seconds=GREATEST(0,EXTRACT(EPOCH FROM (COALESCE(left_at,NOW())-joined_at))::int)
  ...
  await pool.query(
    `INSERT INTO session_attendance (session_id,student_id,attended,attended_at,status,join_timestamp,leave_timestamp,duration_seconds)
     ...
     ON CONFLICT (session_id,student_id) DO UPDATE SET attended=EXCLUDED.attended,attended_at=EXCLUDED.attended_at,status=EXCLUDED.status,
       join_timestamp=EXCLUDED.join_timestamp,leave_timestamp=EXCLUDED.leave_timestamp,duration_seconds=EXCLUDED.duration_seconds,updated_at=NOW()`,
    [sessionId, resolvedStudentId, attended, status, row.joinedAt, row.leftAt, row.durationSeconds],
  );

  if (attended && !previousAttended) {
    await pool.query(
      `UPDATE student_drive_progress
       SET sessions_attended = COALESCE(sessions_attended,0) + 1
       WHERE student_id=$1 AND drive_id = (SELECT drive_id FROM live_sessions WHERE id=$2)`,
